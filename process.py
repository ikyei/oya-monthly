...
def load_tat_lookup() -> dict:
    """Load tat_lookup.json from GitHub repo. Returns empty dict if not found."""
    cfg = CONFIG["GITHUB"]
...
def save_tat_lookup(lookup: dict):
    """Commit tat_lookup.json to GitHub repo data/ folder."""
    cfg = CONFIG["GITHUB"]
    url = f"https://api.github.com/repos/{cfg['OWNER']}/{cfg['REPO']}/contents/data/tat_lookup.json"
    headers = {"Authorization": f"token {cfg['TOKEN']}", "Accept": "application/vnd.github.v3+json"}
    content = base64.b64encode(json.dumps(lookup, ensure_ascii=False).encode("utf-8")).decode("utf-8")
    ...
    try:
        ...
        log.info(f"  TAT lookup saved to GitHub ({total} total entries)")
    except Exception as e:
        log.warning(f"  Could not save TAT lookup to GitHub: {e}")
    ...
    for r in range(header_row_idx + 1, n_rows):
        ...
        if "operating income" in lbl and "total" not in lbl:
            ...
            for r2 in range(r + 1, n_rows):
                raw2 = str(cell(r2, 1)).strip().lower()
