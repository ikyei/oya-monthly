...
CONFIG = {
    ...
    "TAT_LOOKUP_FNAME": "tat_lookup.json",  # stored in 00_OUTPUT Drive folder
...
def load_tat_lookup() -> dict:
    """Load tat_lookup.json — from local file if it exists, else from GitHub."""
    local_path = "data/tat_lookup.json"
    if os.path.exists(local_path):
        try:
            with open(local_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            total = sum(len(v) for v in data.values())
            log.info(f"  TAT lookup loaded from local file ({total} entries)")
            return data
        except Exception as e:
            log.warning(f"  Could not read local TAT lookup: {e}")
    # Fall back to GitHub if local not found (first run on a fresh checkout)
    cfg = CONFIG["GITHUB"]
    url = f"https://api.github.com/repos/{cfg['OWNER']}/{cfg['REPO']}/contents/data/tat_lookup.json"
    ...
    try:
        ...
        total = sum(len(v) for v in data.values())
        log.info(f"  TAT lookup loaded from GitHub ({total} entries)")
        return data
    except Exception as e:
        log.warning(f"  Could not load TAT lookup from GitHub: {e}")
        return {}

def save_tat_lookup(lookup: dict):
    """Write tat_lookup.json locally so the workflow git commit picks it up."""
    os.makedirs("data", exist_ok=True)
    path = "data/tat_lookup.json"
    with open(path, "w", encoding="utf-8") as f:
        json.dump(lookup, f, ensure_ascii=False)
    total = sum(len(v) for v in lookup.values())
    log.info(f"  TAT lookup saved locally ({total} total entries)")
...
def get_spreadsheet_files(drive, folder_id):
    ...
    files = [
        f for f in items
        if f["name"].lower().endswith((".xlsx", ".xls", ".xlt", ".xltx", ".xltm", ".csv"))
...
def get_sheet_as_df(sheets_svc, spreadsheet_id, tab_name):
    """Read a Sheet tab into a DataFrame. Pads short rows to match header length."""
    data = sheets_read(sheets_svc, spreadsheet_id, tab_name)
    if not data or len(data) < 2:
        return pd.DataFrame()
    headers = data[0]
    n = len(headers)
    padded = [row + [""] * (n - len(row)) for row in data[1:]]
...
def upsert_sheet_row(sheets_svc, spreadsheet_id, tab_name, match_cols, row_data):
    ...
    Automatically adds any new columns from row_data that don't yet exist in the header.
    ...
    # Add any new columns from row_data that don't exist in header yet
    new_cols = [k for k in row_data if k not in headers]
    if new_cols:
        headers = headers + new_cols
        # Write updated header row back to sheet
        sheets_write(sheets_svc, spreadsheet_id, f"{tab_name}!A1",
                     [headers])
        log.info(f"    Added new columns to {tab_name}: {new_cols}")
    ...
    values = [["" if row_data.get(h) is None else str(row_data.get(h, "")) for h in headers]]
    if row_idx:
...
def identify_file_type(df: pd.DataFrame) -> str:
    ...
    # P&L: col A or B has 'exchange rate' AND 'interest income' within first 15 rows
    sample = df.iloc[:15]
    for col_idx in range(min(3, df.shape[1])):
        col_vals = sample.iloc[:, col_idx].astype(str).str.lower()
        has_exchange = col_vals.str.contains("exchange rate").any()
        has_interest = col_vals.str.contains("interest income").any()
...
def parse_pl(drive, file_meta, month_str: str) -> dict:
    ...
    # Step 1b: detect label column (col with 'interest income' or 'operating income')
    label_col = 1  # default: col B (index 1)
    for r in range(min(20, n_rows)):
        for c in range(min(5, n_cols)):
            v = str(cell(r, c)).strip().lower()
            if "interest income" in v or "operating income" in v:
                label_col = c
                break
        if label_col != 1:
            break
    log.info(f"    P&L label column (0-based): {label_col}")
    ...
    for r in range(min(10, n_rows)):
        if "exchange rate" in str(cell(r, label_col)).lower():
            if lc_col >= 0:
    ...
    result = {
        ...
        "groupDividends": 0, "priorGroupDividends": 0,
        "opexLines":      [],
    }
    ...
    for r in range(header_row_idx + 1, n_rows):
        raw = str(cell(r, label_col)).strip()
        lbl = raw.lower()
        if not raw:
            continue
        ...
        if "operating income" in lbl and "total" not in lbl:
            ...
            # Scan for Group Fees section, then find Dividends within it only
            in_group_fees = False
            for r2 in range(r + 1, n_rows):
                raw2 = str(cell(r2, label_col)).strip().lower()
                if "group fees" in raw2:
                    in_group_fees = True
                    continue
                if in_group_fees:
                    # Stop if we hit the next major section
                    if any(s in raw2 for s in ["debt servicing", "balance sheet", "cash flow", "beginning balance"]):
                        break
                    if raw2 == "dividends":
                        result["groupDividends"]      = abs(cv(r2))
                        result["priorGroupDividends"] = abs(pv(r2))
                        break
            break
    ...
    result["opex"]         = result["totalOpexLine"]      + result["interestExpense"]      + result["groupDividends"]
    result["priorOpex"]    = result["priorTotalOpexLine"] + result["priorInterestExpense"] + result["priorGroupDividends"]

    # Recompute OI from derived figures — not read from file
    # All costs are already negative, so: OI = revenue + provision + opex
    result["operatingIncome"]      = result["revenue"]      + result["provision"]      + result["opex"]
    result["priorOperatingIncome"] = result["priorRevenue"] + result["priorProvision"] + result["priorOpex"]
...
def parse_app_file(drive, file_meta, month_str: str) -> dict | None:
    ...
    # Detect file/docket number and track number columns
    filenum_idx = -1
    track_idx_app = -1
    for i, h in enumerate(headers):
        if h in ("docket #", "docket#", "filenumber", "file_number", "file number",
                 "file no", "file_no", "loan_id", "loanid", "account_number"):
            filenum_idx = i
        if h in ("track #", "track#", "track_no", "trackno", "track"):
            track_idx_app = i

    total_apps = 0
    teams, areas = set(), set()
    file_entries = {}  # file_num -> app_date_str (for TAT lookup)
    ...
    for _, row in df.iterrows():
        ...
        if area and area.lower() not in ("nan", ""):
            areas.add(area)
        if filenum_idx >= 0 and filenum_idx < len(vals):
            fn = str(vals[filenum_idx]).strip()
            tr = str(vals[track_idx_app]).strip() if track_idx_app >= 0 and track_idx_app < len(vals) else ""
            if fn and fn.lower() not in ("nan", ""):
                key = f"{fn}_{tr}" if tr and tr.lower() not in ("nan", "") else fn
                file_entries[key] = d.strftime("%Y-%m-%d")
...
def parse_disb_file(drive, file_meta, month_str: str, exchange_rate: float) -> dict | None:
    ...
    # Priority date columns — pick the first match in order of preference
    DATE_PRIORITY = ["input_date", "disbursement_date", "date_granted", "disburse_date",
                     "loan_date", "date_of_grant", "date_disbursed"]
    found_exact_date = False
    found_exact_team = False
    for i, h in enumerate(headers):
        if not found_exact_date:
            if h in DATE_PRIORITY:
                date_idx = i
                found_exact_date = True
            elif "date" in h and "expir" not in h and "deduct" not in h and "repay" not in h:
                date_idx = i  # fallback: first generic date col (not deduction/repayment)
    ...
    # Detect file number and track number columns
    filenum_idx = -1
    track_idx_disb = -1
    for i, h in enumerate(headers):
        if h in ("filenumber", "file_number", "file number", "file no", "file_no",
                 "docket #", "docket#", "loan_id", "loanid"):
            filenum_idx = i
        if h in ("track_no", "trackno", "track #", "track#", "track"):
            track_idx_disb = i

    total_local, count = 0.0, 0
    teams = set()
    disb_entries = []  # list of (file_num, disb_date) for TAT calculation
    ...
    for _, row in df.iterrows():
        ...
        if d.month == month and d.year == year:
            ...
            if filenum_idx >= 0 and filenum_idx < len(vals):
                fn = str(vals[filenum_idx]).strip()
                tr = str(vals[track_idx_disb]).strip() if track_idx_disb >= 0 and track_idx_disb < len(vals) else ""
                if fn and fn.lower() not in ("nan", ""):
                    key = f"{fn}_{tr}" if tr and tr.lower() not in ("nan", "") else fn
                    disb_entries.append((key, d))
...
def process_country_month(drive, sheets_svc, ss_id: str, cc: str, month_str: str, folder_id: str) -> dict | None:
    ...
    # P&L — wrapped separately so a bad P&L doesn't kill Proviso metrics
    pl = None
    if identified["pl"]:
        try:
            ...
            log.warning(f"  P&L FAILED: {pl_err} — continuing with Proviso files only")
            pl = None
    else:
        log.info("  No P&L file — continuing with Proviso files only")

    # Use FX rate from P&L if available, else fall back to prior month's stored rate
    if pl:
        fx = pl["exchangeRate"]
    else:
        prior_fx = _get_prior_row(sheets_svc, ss_id, cc, month_str).get("exchange_rate")
        fx = float(prior_fx) if prior_fx else 1.0
        if fx == 1.0:
            log.warning(f"  {cc}: No FX rate available — disbursements will be in local currency")
        else:
            log.info(f"  {cc}: Using prior month FX rate: {fx}")
    ...
    disb = parse_disb_file(drive, identified["disb"], month_str, fx)    if identified["disb"] else None
...
def process_month(month_str: str):
    ...
    # Load TAT lookup table from Drive
    log.info("Loading TAT lookup table...")
    tat_lookup = load_tat_lookup()
    tat_updated = False
    ...
    for cc in CONFIG["COUNTRIES"]:
        ...
        try:
            data = process_country_month(drive, sheets_svc, ss_id, cc, month_str, cc_folder_id)
            if data:
                # Update TAT lookup — key is FileNumber_Track, value is app_date
                app = data.get("app") or {}
                file_entries = app.get("fileEntries", {})
                if file_entries:
                    cc_lookup = tat_lookup.setdefault(cc, {})
                    new_count = 0
                    for key, app_date in file_entries.items():
                        if key not in cc_lookup:
                            cc_lookup[key] = app_date
                            new_count += 1
                    if new_count:
                        log.info(f"    TAT lookup: +{new_count} new entries for {cc}")
                        tat_updated = True

                # Calculate avg TAT for this month
                disb = data.get("disb") or {}
                disb_entries = disb.get("disbEntries", [])
                log.info(f"    TAT debug: disb_entries={len(disb_entries)}, lookup_entries={len(tat_lookup.get(cc, {}))}")
                median_tat = calc_avg_tat(cc, disb_entries, tat_lookup)
                log.info(f"    TAT debug: median_tat={median_tat}")
                data["medianTatDays"] = median_tat
    ...
    # Save updated TAT lookup back to Drive
    if tat_updated:
        save_tat_lookup(tat_lookup)
...
def calc_avg_tat(cc: str, disb_entries: list, lookup: dict) -> float | None:
    """Calculate average turnaround time (days) for loans disbursed this month.
    Each entry in disb_entries is (composite_key, disb_date) where
    composite_key = FileNumber_Track — uniquely identifies a single loan."""
    cc_lookup = lookup.get(cc, {})
    if not cc_lookup or not disb_entries:
        return None
    gaps, unmatched = [], 0
    for key, disb_date in disb_entries:
        if isinstance(disb_date, datetime):
            disb_date = disb_date.date()
        app_date_str = cc_lookup.get(key)
        if not app_date_str:
            unmatched += 1
            continue
        try:
            app_date = datetime.strptime(app_date_str, "%Y-%m-%d").date()
            gap = (disb_date - app_date).days
            if 0 <= gap <= 365:  # sanity check
                gaps.append(gap)
        except Exception:
            continue
    total = len(disb_entries)
    matched = len(gaps)
    match_rate = matched / total if total else 0
    if not matched:
        log.info(f"    TAT {cc}: 0/{total} matched")
        return None
    gaps.sort()
    mid = matched // 2
    median = gaps[mid] if matched % 2 else (gaps[mid - 1] + gaps[mid]) // 2
    log.info(f"    TAT {cc}: {matched}/{total} matched ({match_rate:.0%}), median={median} days")
    return median
...
def store_results(sheets_svc, ss_id: str, cc: str, month_str: str, data: dict):
    ...
    row = {
        "month":                  month_str,
        "exchange_rate":          pl.get("exchangeRate", 0),
        ...
        "revenue_usd":            pl.get("revenue", 0),
        "prior_revenue_usd":      pl.get("priorRevenue", 0),
        "provision_usd":          pl.get("provision", 0),
        "prior_provision_usd":    pl.get("priorProvision", 0),
        "opex_usd":               pl.get("opex", 0),
        "prior_opex_usd":         pl.get("priorOpex", 0),
        "operating_income_usd":   pl.get("operatingIncome", 0),
        "prior_operating_income_usd": pl.get("priorOperatingIncome", 0),
        "interest_income_usd":    pl.get("interestIncome", 0),
        "prior_interest_income_usd": pl.get("priorInterestIncome", 0),
        "processing_fees_usd":    pl.get("processingFees", 0),
        "prior_processing_fees_usd": pl.get("priorProcessingFees", 0),
        "staff_cost_usd":         pl.get("staffCost", 0),
        "prior_staff_cost_usd":   pl.get("priorStaffCost", 0),
        "fuel_cost_usd":          pl.get("fuelCost", 0),
        "prior_fuel_cost_usd":    pl.get("priorFuelCost", 0),
        "vehicle_cost_usd":       pl.get("vehicleCost", 0),
        "prior_vehicle_cost_usd": pl.get("priorVehicleCost", 0),
        "median_tat_days":            data.get("medianTatDays"),
        "prior_median_tat_days":      pr.get("median_tat_days"),
    }
...
def _commit_to_github(month_str: str, payload: dict):
    """Write data/{month_str}.json locally so the workflow git commit picks it up."""
    os.makedirs("data", exist_ok=True)
    path = f"data/{month_str}.json"
    with open(path, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2)
    log.info(f"  ✓ Wrote {path} locally (will be committed by workflow)")
