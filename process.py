    ...
    for r in range(header_row_idx + 1, n_rows):
        ...
        if state == OPEX:
            ...
            if c_v != 0 or p_v != 0:
                ...
                for key in CONFIG["PL_STAFF_KEYS"]:
                    if key in lbl:
                        result["staffCost"] += c_v; result["priorStaffCost"] += p_v
                for key in CONFIG["PL_FUEL_KEYS"]:
                    if key in lbl:
                        result["fuelCost"] += c_v; result["priorFuelCost"] += p_v
                for key in CONFIG["PL_VEHICLE_KEYS"]:
                    if key in lbl:
