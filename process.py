...
def load_tat_lookup(drive) -> dict:
    """Download tat_lookup.json from 00_OUTPUT folder. Returns empty dict if not found."""
    output_folder = find_subfolder(drive, CONFIG["DRIVE_ROOT"], "00_OUTPUT")
    if not output_folder:
        return {}
    items = list_folder(drive, output_folder)
    meta = next((f for f in items if f["name"] == CONFIG["TAT_LOOKUP_FNAME"]), None)
    if not meta:
        return {}
    try:
        fh = io.BytesIO()
        ...
        log.warning(f"  Could not load TAT lookup: {e}")
        return {}

def save_tat_lookup(drive, lookup: dict):
    """Upload/update tat_lookup.json in 00_OUTPUT folder."""
    output_folder = find_subfolder(drive, CONFIG["DRIVE_ROOT"], "00_OUTPUT")
    if not output_folder:
        log.warning("  00_OUTPUT folder not found — TAT lookup not saved")
        return
    items = list_folder(drive, output_folder)
    existing = next((f for f in items if f["name"] == CONFIG["TAT_LOOKUP_FNAME"]), None)
    content = json.dumps(lookup, ensure_ascii=False).encode("utf-8")
    ...
    if existing:
        ...
        meta = {"name": CONFIG["TAT_LOOKUP_FNAME"], "parents": [output_folder]}
        drive.files().create(body=meta, media_body=media).execute()
    log.info(f"  TAT lookup saved ({sum(len(v) for v in lookup.values())} total entries)")
...
def process_month(month_str: str):
    ...
    tat_lookup = load_tat_lookup(drive)
    tat_updated = False
    ...
    if tat_updated:
        save_tat_lookup(drive, tat_lookup)
