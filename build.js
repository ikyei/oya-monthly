...
async function buildPptx(monthStr, data) {
  ...
  // ── SLIDE 2: LOAN APPLICATIONS ──────────────────────────────────────────
  {
    ...
    const tatVals = CC.filter(cc => d[cc]?.median_tat_days != null).map(cc => d[cc].median_tat_days);
    const grpTat  = tatVals.length ? Math.round(tatVals.reduce((a,b) => a+b, 0) / tatVals.length) : null;
    const grpTatStr = grpTat != null ? grpTat + " days" : "—";

    kpiBox(s, 8.7, 0.75, 4.45, 1.0, "Total Applications", fmtNum(totalApps));
    kpiBox(s, 8.7, 1.85, 4.45, 1.0, "Group Avg per Team", fmtNum(groupAppsTeam));
    kpiBox(s, 8.7, 2.95, 4.45, 1.0, "Change vs Prior Month", fmtChg(groupAppsTeam, priorGroupAppsTeam));
    kpiBox(s, 8.7, 4.05, 4.45, 1.0, "Avg Turnaround Time", grpTatStr);
    commentaryBox(s, 8.7, 5.15, 4.45, 2.02, d.commentary?.apps);
  }
  ...
  // ── SLIDE 4: DISBURSEMENTS ───────────────────────────────────────────────
  {
    ...
    kpiBox(s, 8.7, 2.0,  4.45, 1.15, "Group Disb per Team", fmtNum(grpDisbPerTeam));
  ...
  const monthSections = allMonths.map(({ monthStr, data: d }) => {
    ...
    const appRows = [...CC].sort((a,b) => (d[b]?.apps_per_team||0) - (d[a]?.apps_per_team||0)).map(cc => `
      <tr>
        ...
        <td class="${chgClass(d[cc]?.median_tat_days, d[cc]?.prior_median_tat_days, true)}">${fmtC(d[cc]?.median_tat_days, d[cc]?.prior_median_tat_days)}</td>
      </tr>`).join("");
    ...
    const appTotalRow = `<tr class="total-row">
      ...
      <td class="${chgClass(grpAvgTat, grpPriorAvgTat, true)}">${fmtC(grpAvgTat, grpPriorAvgTat)}</td>
    </tr>`;
    ...
    const grpPriorDisbPerTeamWt = grpPriorTeams ? Math.round(grpPriorTotalLoans / grpPriorTeams) : 0;
    ...
    <div class="month-page" id="page-${monthStr}">
      ...
      <div class="table-block">
        ...
        <div class="tbl-wrap"><table>
          <thead><tr><th>Country</th><th>Apps / Team</th><th>vs Prior Month</th><th>Applications</th><th>Approvals</th><th>Approval Rate</th><th>vs Last Month</th></tr></thead>
      ...
      <div class="table-block">
        ...
        <div class="tbl-wrap"><table>
          <thead><tr><th>Country</th><th>Avg Turnaround Time</th><th>Trend</th><th>Loan Book</th><th>vs Prior</th></tr></thead>
      ...
      <div class="table-block">
        ...
        <div class="tbl-wrap"><table>
          <thead><tr><th>Country</th><th>Chronic Rate</th><th>vs Last Month</th><th>PAR 30</th><th>vs Last Month</th></tr></thead>
