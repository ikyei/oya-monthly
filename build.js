...
function buildCommentary(d) {
  ...
  const tatLines = tatCCs.length ? (() => {
    ...
    const grpMedian = tatCCs.length % 2
      ? [...tatCCs].sort((a,b) => d[a].median_tat_days - d[b].median_tat_days)[Math.floor(tatCCs.length/2)].median_tat_days
      : null;
    const lines = [`Group median turnaround: ${grpMedian != null ? grpMedian + " days" : "N/A"}.`];
    if (fastest !== slowest) {
...
async function buildPptx(monthStr, data) {
  ...
  // ── SLIDE 6: TURNAROUND TIME ─────────────────────────────────────────────
  {
    ...
    sectionLabel(s, 0, 0.59, W, `Median Days from Application to Disbursement – ${RM}`);
    ...
    kpiBox(s, 8.7, 0.75, 4.45, 1.15, "Group Median Turnaround", grpTatMedian != null ? grpTatMedian + " days" : "—");
