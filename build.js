...
function kpiBox(slide, x, y, w, h, label, value, valueColor) {
  ...
  slide.addText(String(value), { x, y: y + 0.1, w, h: h * 0.55, fontSize: 26, bold: true, color: valueColor || C.brown, fontFace: FONT, align: "center", valign: "middle" });
...
function buildCommentary(d) {
  ...
  const tatCCs = CC.filter(cc => d[cc]?.median_tat_days != null);
  const tatLines = tatCCs.length ? (() => {
    const fastest = tatCCs.reduce((b, cc) => (d[cc].median_tat_days < d[b].median_tat_days ? cc : b), tatCCs[0]);
    const slowest = tatCCs.reduce((b, cc) => (d[cc].median_tat_days > d[b].median_tat_days ? cc : b), tatCCs[0]);
    const sortedTat = [...tatCCs].sort((a, b) => d[a].median_tat_days - d[b].median_tat_days);
    const mid = Math.floor(sortedTat.length / 2);
    const grpMedian = sortedTat.length % 2
      ? d[sortedTat[mid]].median_tat_days
      : Math.round((d[sortedTat[mid - 1]].median_tat_days + d[sortedTat[mid]].median_tat_days) / 2);
    const lines = [`Group avg turnaround: ${grpMedian + " days"}.`];
    if (fastest !== slowest) {
      lines.push(`Fastest: ${CC_NAMES[fastest]} (${d[fastest].median_tat_days} days). Slowest: ${CC_NAMES[slowest]} (${d[slowest].median_tat_days} days).`);
    }
    return lines;
  })() : ["Turnaround time data not yet available."];
...
async function buildPptx(monthStr, data) {
  ...
  // Helper: get change color. invert=true means lower is better (PAR, chronic, expenses)
  const chgColor = (curr, prior, invert) => {
    if (!curr || !prior) return C.slateLight;
    const up = curr > prior;
    return (up !== !!invert) ? C.teal : C.red;
  };

  // Helper: return CC sorted by a JSON key (desc by default, asc=true for lower-is-better)
  const sortCC = (key, asc = false) => [...CC].sort((a, b) =>
    asc ? (d[a]?.[key] || 0) - (d[b]?.[key] || 0)
        : (d[b]?.[key] || 0) - (d[a]?.[key] || 0)
  );

  // Pre-compute disb order (slide 4) so slide 5 reuses it
  const disbOrder = sortCC("total_disb_usd");
  ...
  // ── SLIDE 2: LOAN APPLICATIONS ──────────────────────────────────────────
  {
    ...
    const S2 = sortCC("apps_per_team");
    const appsVals  = S2.map(cc => d[cc]?.apps_per_team || 0);
    const appsChgs  = S2.map(cc => fmtChg(d[cc]?.apps_per_team, d[cc]?.prior_apps_per_team));
    const appsColors = S2.map(cc => chgColor(d[cc]?.apps_per_team, d[cc]?.prior_apps_per_team, false));

    barChart(s, 0.15, 0.92, 8.3, 5.1, S2.map(c => CC_NAMES[c]), appsVals, C.slate, false);
    ...
    const priorGroupAppsTeam = priorTotalTeams && priorTotalApps
      ? Math.round(priorTotalApps / priorTotalTeams)
      : totalTeams
        ? Math.round(CC.reduce((s,cc)=>s+(d[cc]?.prior_apps_per_team||0)*(d[cc]?.num_teams||0),0) / totalTeams)
        : 0;
  ...
  // ── SLIDE 3: ASSESSMENT EFFICIENCY ──────────────────────────────────────
  {
    ...
    const S3 = sortCC("approval_rate");
    const effVals   = S3.map(cc => d[cc]?.approval_rate || 0);
    const effArrows = S3.map(cc => { const c=d[cc]?.approval_rate, p=d[cc]?.prior_approval_rate; return (!c||!p||c===p)?"—":c>p?"↑":"↓"; });
    const effColors = S3.map(cc => chgColor(d[cc]?.approval_rate, d[cc]?.prior_approval_rate, false));

    barChart(s, 0.15, 0.92, 8.3, 5.1, S3.map(c => CC_NAMES[c]), effVals, C.teal, true);
    sectionLabel(s, 0, 6.1, 8.5, "Trend vs Prior Month");
    changeRow(s, effArrows, effColors, 6.48, 8.3 / 5, 0.15);
    ...
    const priorGroupEff = priorTotalApprovals && priorTotalAppsEff
      ...
      : totalApps
        ? CC.reduce((s,cc)=>s+(d[cc]?.prior_approval_rate||0)*(d[cc]?.total_apps||0),0)/totalApps
        : 0;
    const effArrow = (!groupEff||!priorGroupEff||groupEff===priorGroupEff)?"—":groupEff>priorGroupEff?"↑":"↓";
    const effArrowColor = groupEff > priorGroupEff ? C.teal : C.red;
    ...
    kpiBox(s, 8.7, 3.15, 4.45, 1.1, "Trend vs Prior Month", effArrow, effArrowColor);
  ...
  // ── SLIDE 4: DISBURSEMENTS ───────────────────────────────────────────────
  {
    ...
    const disbVals   = disbOrder.map(cc => d[cc]?.total_disb_usd || 0);
    const disbChgs   = disbOrder.map(cc => fmtChg(d[cc]?.total_disb_usd, d[cc]?.prior_total_disb_usd));
    const disbColors = disbOrder.map(cc => chgColor(d[cc]?.total_disb_usd, d[cc]?.prior_total_disb_usd, false));

    barChart(s, 0.15, 0.92, 8.3, 5.1, disbOrder.map(c => CC_NAMES[c]), disbVals, C.slate, false);
    sectionLabel(s, 0, 6.1, 8.5, "% Change from Prior Month");
    ...
    // Derive prior loan count per country using prior avg loan size
    const priorTotalLoansDisb = CC.reduce((s, cc) => {
      const avg = d[cc]?.prior_avg_loan_size_usd, disb = d[cc]?.prior_total_disb_usd;
      return s + (avg && disb ? Math.round(disb / avg) : 0);
    }, 0);
    const priorTeamsDenom = priorTotalTeamsDisb || totalTeamsDisb;
    const priorGrpDisbPerTeam = priorTeamsDenom && priorTotalLoansDisb ? Math.round(priorTotalLoansDisb / priorTeamsDenom) : 0;
    const totalLoansDisb = CC.reduce((s, cc) => {
      const avg = d[cc]?.avg_loan_size_usd, disb = d[cc]?.total_disb_usd;
      return s + (avg && disb ? Math.round(disb / avg) : 0);
    }, 0);
    const grpDisbPerTeam = totalTeamsDisb && totalLoansDisb ? Math.round(totalLoansDisb / totalTeamsDisb) : 0;

    kpiBox(s, 8.7, 0.75, 4.45, 1.15, "Total Disbursed (USD)", fmtUsd(totalDisb));
    kpiBox(s, 8.7, 2.0,  4.45, 1.15, "Change vs Prior Month", fmtChg(totalDisb, priorTotalDisb));
    kpiBox(s, 8.7, 3.25, 4.45, 0.8,  "Group Disb per Team", fmtNum(grpDisbPerTeam));

    // Teams per country grid
    sectionLabel(s, 8.7, 4.05, 4.45, "Number of Teams");
    disbOrder.forEach((cc, i) => {
      const col = i % 3, row = Math.floor(i / 3);
      const x = 8.7 + col * 1.48, y = 4.4 + row * 0.65;
      s.addShape("rect", { x, y, w: 1.35, h: 0.58, fill: { color: C.cream }, line: { color: C.lineGray, pt: 0.5 } });
      s.addText(CC_NAMES[cc], { x, y, w: 1.35, h: 0.26, fontSize: 11, color: C.slateLight, fontFace: FONT2, align: "center", valign: "bottom" });
      s.addText(fmtNum(d[cc]?.num_teams), { x, y: y + 0.26, w: 1.35, h: 0.32, fontSize: 11, bold: true, color: C.brown, fontFace: FONT, align: "center", valign: "middle" });
    });

    commentaryBox(s, 8.7, 5.82, 4.45, 1.05, d.commentary?.disbursements);
  }
  ...
  // ── SLIDE 5: AVG LOAN SIZE & DISB PER TEAM ──────────────────────────────
  {
    ...
    const loanVals   = disbOrder.map(cc => d[cc]?.avg_loan_size_usd || 0);
    const loanChgs   = disbOrder.map(cc => fmtChg(d[cc]?.avg_loan_size_usd, d[cc]?.prior_avg_loan_size_usd));
    const loanColors = disbOrder.map(cc => chgColor(d[cc]?.avg_loan_size_usd, d[cc]?.prior_avg_loan_size_usd, false));

    const disbTeamVals   = disbOrder.map(cc => d[cc]?.disb_per_team || 0);
    const disbTeamChgs   = disbOrder.map(cc => fmtChg(d[cc]?.disb_per_team, d[cc]?.prior_disb_per_team));
    const disbTeamColors = disbOrder.map(cc => chgColor(d[cc]?.disb_per_team, d[cc]?.prior_disb_per_team, false));

    barChart(s, 0.15, 0.92, 6.35, 5.1, disbOrder.map(c => CC_NAMES[c]), loanVals, C.slate, false);
    barChart(s, 6.75, 0.92, 6.35, 5.1, disbOrder.map(c => CC_NAMES[c]), disbTeamVals, C.teal, false);
  ...
  // ── SLIDE 6: TURNAROUND TIME ─────────────────────────────────────────────
  {
    const s = prs.addSlide();
    slideHeader(s, "Turnaround Time", RM);
    sectionLabel(s, 0, 0.59, W, `Avg Days from Application to Disbursement – ${RM}`);

    // Sort lowest to highest (fastest country first)
    const S_TAT = [...CC].sort((a, b) =>
      (d[a]?.median_tat_days ?? 999) - (d[b]?.median_tat_days ?? 999)
    );
    const tatVals   = S_TAT.map(cc => d[cc]?.median_tat_days ?? 0);
    const tatArrows = S_TAT.map(cc => {
      const c = d[cc]?.median_tat_days, p = d[cc]?.prior_median_tat_days;
      return (c == null || p == null || c === p) ? "—" : c < p ? "↓" : "↑";
    });
    const tatColors = S_TAT.map(cc => chgColor(d[cc]?.median_tat_days, d[cc]?.prior_median_tat_days, true));

    barChart(s, 0.15, 0.92, 8.3, 5.1, S_TAT.map(c => CC_NAMES[c]), tatVals, C.teal, false);
    sectionLabel(s, 0, 6.1, 8.5, "Trend vs Prior Month");
    changeRow(s, tatArrows, tatColors, 6.48, 8.3 / 5, 0.15);

    // Group median (simple median across country medians)
    const tatAll = CC.map(cc => d[cc]?.median_tat_days).filter(v => v != null).sort((a,b) => a-b);
    const grpTatMedian = tatAll.length
      ? (tatAll.length % 2 ? tatAll[Math.floor(tatAll.length/2)]
          : Math.round((tatAll[tatAll.length/2-1] + tatAll[tatAll.length/2]) / 2))
      : null;
    const priorTatAll = CC.map(cc => d[cc]?.prior_median_tat_days).filter(v => v != null).sort((a,b) => a-b);
    const grpPriorTatMedian = priorTatAll.length
      ? (priorTatAll.length % 2 ? priorTatAll[Math.floor(priorTatAll.length/2)]
          : Math.round((priorTatAll[priorTatAll.length/2-1] + priorTatAll[priorTatAll.length/2]) / 2))
      : null;

    const grpTatArrow = (grpTatMedian != null && grpPriorTatMedian != null && grpTatMedian !== grpPriorTatMedian)
      ? (grpTatMedian < grpPriorTatMedian ? "↓" : "↑") : "—";
    const grpTatArrowColor = (grpTatMedian != null && grpPriorTatMedian != null)
      ? (grpTatMedian < grpPriorTatMedian ? C.teal : C.red) : C.slateLight;

    kpiBox(s, 8.7, 0.75, 4.45, 1.15, "Group Avg Turnaround Time", grpTatMedian != null ? grpTatMedian + " days" : "—");
    kpiBox(s, 8.7, 2.0,  4.45, 1.15, "Trend vs Prior Month", grpTatArrow, grpTatArrowColor);
    commentaryBox(s, 8.7, 3.25, 4.45, 3.92, d.commentary?.tat);
  }
  ...
  // ── SLIDE 7: LOAN BOOK ───────────────────────────────────────────────────
  {
    ...
    const S6 = sortCC("loan_book_usd");
    const bookVals   = S6.map(cc => d[cc]?.loan_book_usd || 0);
    const bookChgs   = S6.map(cc => fmtChg(d[cc]?.loan_book_usd, d[cc]?.prior_loan_book_usd));
    const bookColors = S6.map(cc => chgColor(d[cc]?.loan_book_usd, d[cc]?.prior_loan_book_usd, false));

    barChart(s, 0.15, 0.92, 8.3, 5.1, S6.map(c => CC_NAMES[c]), bookVals, C.slate, false);
    sectionLabel(s, 0, 6.1, 8.5, "% Change from Prior Month");
  ...
  // ── SLIDE 8: CHRONIC RATES ───────────────────────────────────────────────
  {
    ...
    const chronicRows = CC.map(cc => {
      ...
      const arrow = (curr == null || prior == null || curr === prior) ? "—" : curr > prior ? "↑" : "↓";
      const arrowColor = (curr != null && prior != null) ? (curr > prior ? C.red : C.teal) : C.slateLight;
      return [
        { text: CC_NAMES[cc] },
        { text: fmtPct(prior) },
        { text: fmtPct(curr) },
    ...
    const grpChronicArrow = (grpChronic != null && grpPriorChronic != null && grpChronic !== grpPriorChronic) ? (grpChronic > grpPriorChronic ? "↑" : "↓") : "—";
    const grpChronicArrowColor = (grpChronic != null && grpPriorChronic != null) ? (grpChronic > grpPriorChronic ? C.red : C.teal) : C.slateLight;
    chronicRows.sort((a, b) => {
      const av = parseFloat(a[2].text) || 0, bv = parseFloat(b[2].text) || 0;
      return av - bv;
    });
    ...
    chronicRows.push([
      ...
      { text: grpChronicArrow, bold: true, color: grpChronicArrowColor },
    ]);
    countryTable(s, 0.15, 0.95, 13.0, chronicRows,
      [3.5, 3.0, 3.0, 3.5],
      ["Country", "Chronic Rate – Prior Month", "Chronic Rate – Month Under Review", "Trend"]
    );
  }
  ...
  // ── SLIDE 9: PAR 30 ──────────────────────────────────────────────────────
  {
    ...
    const grpParArrow = (grpPar != null && grpPriorPar != null && grpPar !== grpPriorPar) ? (grpPar > grpPriorPar ? "↑" : "↓") : "—";
    const grpParArrowColor = (grpPar != null && grpPriorPar != null) ? (grpPar > grpPriorPar ? C.red : C.teal) : C.slateLight;
    par30Rows.sort((a, b) => {
      const av = parseFloat(a[2].text) || 0, bv = parseFloat(b[2].text) || 0;
      return av - bv;
    });
    ...
    par30Rows.push([
      ...
      { text: grpParArrow, bold: true, color: grpParArrowColor },
    ]);
    countryTable(s, 0.15, 0.95, 13.0, par30Rows,
      [3.5, 3.0, 3.0, 3.5],
      ["Country", "PAR 30 – Prior Month", "PAR 30 – Month Under Review", "Trend"]
    );
  }
  ...
  // ── SLIDE 10: FINANCIAL SUMMARY ───────────────────────────────────────────
  {
    ...
    const plRows = [...CC].sort((a,b) => (d[b]?.operating_income_usd||0) - (d[a]?.operating_income_usd||0)).map(cc => {
      const cd = d[cc] || {};
  ...
  // ── SLIDES 11–13: OPEX BREAKDOWN ────────────────────────────────────────
  const opexSlides = [
    { title: "Operating Expenses \u2013 Staff Cost",          key: "staff_cost_usd",    priorKey: "prior_staff_cost_usd",   color: C.slate, showDisbChg: false },
    { title: "Operating Expenses \u2013 Fuel Cost",           key: "fuel_cost_usd",     priorKey: "prior_fuel_cost_usd",    color: C.amber, showDisbChg: true  },
    { title: "Operating Expenses \u2013 Vehicle Maintenance", key: "vehicle_cost_usd",  priorKey: "prior_vehicle_cost_usd", color: C.red,   showDisbChg: true  },
  ];

  for (const { title, key, priorKey, color, showDisbChg } of opexSlides) {
    const s = prs.addSlide();
    slideHeader(s, title, RM);
    sectionLabel(s, 0, 0.59, W, `Average ${title.split("\u2013")[1].trim()} Per Team by Country (USD) \u2013 ${RM}`);

    // Sort lowest to highest cost per team (most efficient first)
    const SX = [...CC].sort((a, b) => {
      const ta = d[a]?.num_teams || 1, tb = d[b]?.num_teams || 1;
      const va = Math.round(Math.abs(d[a]?.[key] || 0) / ta);
      const vb = Math.round(Math.abs(d[b]?.[key] || 0) / tb);
      return va - vb;
    });

    const teams     = SX.map(cc => d[cc]?.num_teams || 1);
    const vals      = SX.map((cc, i) => teams[i] ? Math.round(Math.abs(d[cc]?.[key]      || 0) / teams[i]) : 0);
    const priorVals = SX.map((cc, i) => teams[i] ? Math.round(Math.abs(d[cc]?.[priorKey] || 0) / teams[i]) : 0);
    const chgs      = SX.map((cc, i) => fmtChg(vals[i], priorVals[i]));
    const chgColors = SX.map((cc, i) => chgColor(vals[i], priorVals[i], true)); // inverted: up = bad

    const chartH = showDisbChg ? 4.7 : 5.2;
    barChart(s, 0.15, 0.92, 13.0, chartH, SX.map(c => CC_NAMES[c]), vals, color, false);
    sectionLabel(s, 0, chartH + 0.97, W, "% Change in Cost per Team vs Prior Month");
    changeRow(s, chgs, chgColors, chartH + 1.3, 13.0 / 5, 0.15);

    if (showDisbChg) {
      const disbVals      = SX.map(cc => d[cc]?.disb_per_team       || 0);
      const priorDisbVals = SX.map(cc => d[cc]?.prior_disb_per_team || 0);
      const disbChgs      = SX.map((cc, i) => fmtChg(disbVals[i], priorDisbVals[i]));
      const disbColors    = SX.map((cc, i) => chgColor(disbVals[i], priorDisbVals[i], false)); // up = good
      sectionLabel(s, 0, chartH + 1.75, W, "% Change in Disb per Team vs Prior Month");
      changeRow(s, disbChgs, disbColors, chartH + 2.08, 13.0 / 5, 0.15);
    }
  }
...
function buildHtml(allMonths) {
  ...
  const monthSections = allMonths.map(({ monthStr, data: d }) => {
    ...
    const appRows = [...CC].sort((a,b) => (d[b]?.apps_per_team||0) - (d[a]?.apps_per_team||0)).map(cc => `
      <tr>
    ...
    const disbRows = [...CC].sort((a,b) => (d[b]?.total_disb_usd||0) - (d[a]?.total_disb_usd||0)).map(cc => `
      <tr>
        <td class="country">${CC_NAMES_FULL[cc]}</td>
        ...
        <td class="${chgClass(d[cc]?.avg_loan_size_usd, d[cc]?.prior_avg_loan_size_usd, false)}">${fmtC(d[cc]?.avg_loan_size_usd, d[cc]?.prior_avg_loan_size_usd)}</td>
        <td>${fmtN(d[cc]?.disb_per_team)}</td>
        <td class="${chgClass(d[cc]?.disb_per_team, d[cc]?.prior_disb_per_team, false)}">${fmtC(d[cc]?.disb_per_team, d[cc]?.prior_disb_per_team)}</td>
      </tr>`).join("");

    // TAT & Loan Book rows — sorted fastest first
    const tatBookRows = [...CC].sort((a,b) => (d[a]?.median_tat_days ?? 999) - (d[b]?.median_tat_days ?? 999)).map(cc => `
      <tr>
        <td class="country">${CC_NAMES_FULL[cc]}</td>
        <td>${d[cc]?.median_tat_days != null ? Math.round(d[cc].median_tat_days) + " days" : "—"}</td>
        <td class="${bpsClass(d[cc]?.median_tat_days, d[cc]?.prior_median_tat_days, true)}">${fmtArrow(d[cc]?.median_tat_days, d[cc]?.prior_median_tat_days, true)}</td>
    ...
    const defRows = [...CC].sort((a,b) => (d[a]?.chronic_rate||0) - (d[b]?.chronic_rate||0)).map(cc => `
      <tr>
        <td class="country">${CC_NAMES_FULL[cc]}</td>
    ...
    // Weight prior_apps_per_team by current num_teams (stable proxy when prior_total_apps missing)
    const grpPriorAppsTeam = grpPriorTeams && grpPriorTotalApps
      ? Math.round(grpPriorTotalApps / grpPriorTeams)
      : grpTotalTeams
        ? Math.round(CC.reduce((s,cc) => s + (d[cc]?.prior_apps_per_team||0) * (d[cc]?.num_teams||0), 0) / grpTotalTeams)
        : 0;
    ...
    // Weight prior_approval_rate by current total_apps (stable proxy when prior_total_approvals missing)
    const grpPriorAppRate = grpPriorApprovals && grpPriorTotalApps
      ? grpPriorApprovals / grpPriorTotalApps
      : grpTotalApps
        ? CC.reduce((s,cc) => s + (d[cc]?.prior_approval_rate||0) * (d[cc]?.total_apps||0), 0) / grpTotalApps
        : 0;

    // Derive num_loans per country from total_disb / avg_loan_size, then sum for group avg
    const grpTotalLoans = CC.reduce((s, cc) => {
      const avg = d[cc]?.avg_loan_size_usd;
      const disb = d[cc]?.total_disb_usd;
      return s + (avg && disb ? Math.round(disb / avg) : 0);
    }, 0);
    const grpAvgLoanSize = grpTotalLoans ? totDisb / grpTotalLoans : 0;
    const grpDisbPerTeam = grpTotalTeams && grpTotalLoans ? Math.round(grpTotalLoans / grpTotalTeams) : 0;
    ...
    const tatCCs = CC.filter(cc => d[cc]?.median_tat_days != null);
    const grpAvgTat = tatCCs.length ? Math.round(tatCCs.reduce((s,cc) => s + d[cc].median_tat_days, 0) / tatCCs.length) : null;
    const priorTatCCs = CC.filter(cc => d[cc]?.prior_median_tat_days != null);
    const grpPriorAvgTat = priorTatCCs.length ? Math.round(priorTatCCs.reduce((s,cc) => s + d[cc].prior_median_tat_days, 0) / priorTatCCs.length) : null;
    ...
    const grpPriorTotalLoans = CC.reduce((s, cc) => {
      const avg = d[cc]?.prior_avg_loan_size_usd, disb = d[cc]?.prior_total_disb_usd;
      return s + (avg && disb ? Math.round(disb / avg) : 0);
    }, 0);
    const grpPriorAvgLoanSize = grpPriorTotalLoans ? grpPriorDisb / grpPriorTotalLoans : 0;
    const grpPriorTeamsDenom = grpPriorTeams || grpTotalTeams;
    const grpPriorDisbPerTeamWt = grpPriorTeamsDenom ? Math.round(grpPriorTotalLoans / grpPriorTeamsDenom) : 0;

    const disbTotalRow = `<tr class="total-row">
      <td>Group Total</td>
      <td>${fmtU(totDisb)}</td>
      <td class="${chgClass(totDisb, grpPriorDisb, false)}">${fmtC(totDisb, grpPriorDisb)}</td>
      <td>${grpAvgLoanSize ? fmtU(grpAvgLoanSize) : "—"}</td>
      <td class="${chgClass(grpAvgLoanSize, grpPriorAvgLoanSize, false)}">${fmtC(grpAvgLoanSize, grpPriorAvgLoanSize)}</td>
      <td>${fmtN(grpDisbPerTeam)}</td>
      <td class="${chgClass(grpDisbPerTeam, grpPriorDisbPerTeamWt, false)}">${fmtC(grpDisbPerTeam, grpPriorDisbPerTeamWt)}</td>
    </tr>`;

    const tatBookTotalRow = `<tr class="total-row">
      <td>Group Total</td>
      <td>${grpAvgTat != null ? grpAvgTat + " days" : "—"}</td>
      <td class="${bpsClass(grpAvgTat, grpPriorAvgTat, true)}">${fmtArrow(grpAvgTat, grpPriorAvgTat, true)}</td>
      <td>${fmtU(totBook)}</td>
    ...
    <div class="month-page" id="page-${monthStr}">
      ...
      <div class="kpi-strip">
        ...
        <div class="kpi-card"><div class="kpi-val kpi-val-days">${grpAvgTat != null ? grpAvgTat + " days" : "—"}</div><div class="kpi-lbl">Avg Turnaround Time</div></div>
        ...
        <div class="kpi-card"><div class="kpi-val">${fmtP(grpChronic)}</div><div class="kpi-lbl">Group Chronic Rate</div></div>
      </div>

      <div class="table-block">
        <div class="tbl-title">Loan Applications & Assessment Efficiency</div>
        <div class="tbl-wrap"><table>
          <thead><tr><th>Country</th><th>Apps / Team</th><th>vs Last Month</th><th>Applications</th><th>Approvals</th><th>Approval Rate</th><th>Trend</th></tr></thead>
          <tbody>${appRows}${appTotalRow}</tbody>
        </table></div>
      </div>

      <div class="table-block">
        <div class="tbl-title">Disbursements</div>
        <div class="tbl-wrap"><table class="tbl-t2">
          <thead><tr><th>Country</th><th>Total Disbursed</th><th>vs Prior</th><th>Avg Loan Size</th><th>vs Prior</th><th>Disb / Team</th><th>vs Prior</th></tr></thead>
          <tbody>${disbRows}${disbTotalRow}</tbody>
        </table></div>
      </div>

      <div class="table-block">
        <div class="tbl-title">Turnaround Time &amp; Loan Book</div>
        <div class="tbl-wrap"><table class="tat-table">
          <thead><tr><th>Country</th><th>Avg Turnaround Time</th><th>Trend</th><th>Loan Book</th><th>vs Last Month</th></tr></thead>
          <tbody>${tatBookRows}${tatBookTotalRow}</tbody>
        </table></div>
      </div>

      <div class="table-block">
        <div class="tbl-title">Default Rates</div>
        <div class="tbl-wrap"><table class="tbl-t4">
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Paired columns: tight left padding on companion col to visually group metric+change */
    .tbl-t1 th:nth-child(3), .tbl-t1 td:nth-child(3),
    .tbl-t1 th:nth-child(7), .tbl-t1 td:nth-child(7) { padding-left: 4px; }
    .tbl-t2 th:nth-child(3), .tbl-t2 td:nth-child(3),
    .tbl-t2 th:nth-child(5), .tbl-t2 td:nth-child(5),
    .tbl-t2 th:nth-child(7), .tbl-t2 td:nth-child(7) { padding-left: 4px; }
    .tat-table th:nth-child(3), .tat-table td:nth-child(3),
    .tat-table th:nth-child(5), .tat-table td:nth-child(5) { padding-left: 4px; }
    .tbl-t4 th:nth-child(3), .tbl-t4 td:nth-child(3),
    .tbl-t4 th:nth-child(5), .tbl-t4 td:nth-child(5) { padding-left: 4px; }
    .tat-table th:nth-child(1), .tat-table td:nth-child(1) { width: 30%; }
    .tat-table th:nth-child(2), .tat-table td:nth-child(2) { width: 22%; }
    .tat-table th:nth-child(3), .tat-table td:nth-child(3) { width: 10%; text-align: center; }
    .tat-table th:nth-child(4), .tat-table td:nth-child(4) { width: 22%; }
    .tat-table th:nth-child(5), .tat-table td:nth-child(5) { width: 16%; }
    table { width: 100%; border-collapse: collapse; min-width: 480px; }
    thead tr { background: var(--brown); }
    thead th { padding: 0.55rem 0.8rem; text-align: center; font-size: 0.85rem; font-weight: 600; color: var(--cream); letter-spacing: 0.04em; white-space: nowrap; }
    thead th:first-child { text-align: left; }
    tbody tr:nth-child(even) { background: var(--cream); }
    tbody tr:hover { background: var(--cream-dk); transition: background 0.15s; }
    tbody td { padding: 0.5rem 0.8rem; text-align: center; font-size: 1.0rem; color: var(--text); border-bottom: 1px solid var(--border); }
    ...
    td.pos, td.neg { text-align: center; }
