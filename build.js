...
async function buildPptx(monthStr, data) {
  ...
  // ── SLIDE 6: TURNAROUND TIME ─────────────────────────────────────────────
  {
    ...
    kpiBox(s, 8.7, 0.75, 4.45, 1.15, "Group Median TAT", grpTatMedian != null ? grpTatMedian + " days" : "—");
...
function buildHtml(allMonths) {
  ...
  const monthSections = allMonths.map(({ monthStr, data: d }) => {
    ...
    const tatBookRows = [...CC].sort((a,b) => (d[a]?.median_tat_days ?? 999) - (d[b]?.median_tat_days ?? 999)).map(cc => `
      <tr>
        <td class="country">${CC_NAMES_FULL[cc]}</td>
        <td>${d[cc]?.median_tat_days != null ? Math.round(d[cc].median_tat_days) + "d" : "—"}</td>
    ...
    const tatBookTotalRow = `<tr class="total-row">
      <td>Group Total</td>
      <td>${grpAvgTat != null ? grpAvgTat + "d" : "—"}</td>
    ...
    <div class="month-page" id="page-${monthStr}">
      ...
      <div class="kpi-strip">
        ...
        <div class="kpi-card"><div class="kpi-val">${grpAvgTat != null ? grpAvgTat + "d" : "—"}</div><div class="kpi-lbl">Avg Turnaround Time</div></div>
