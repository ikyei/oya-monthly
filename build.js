...
async function buildPptx(monthStr, data) {
  ...
  // ── SLIDE 2: LOAN APPLICATIONS ──────────────────────────────────────────
  {
    ...
    const priorGroupAppsTeam = priorTotalTeams ? Math.round(priorTotalApps / priorTotalTeams) : 0;
  ...
  // ── SLIDE 3: ASSESSMENT EFFICIENCY ──────────────────────────────────────
  {
    ...
    const priorGroupEff  = priorTotalAppsEff ? priorTotalApprovals / priorTotalAppsEff : 0;
...
function buildHtml(allMonths) {
  ...
  const monthSections = allMonths.map(({ monthStr, data: d }) => {
    ...
    const grpPriorAppsTeam = grpPriorTeams ? Math.round(grpPriorTotalApps / grpPriorTeams) : 0;
    ...
    const grpPriorAppRate = grpPriorTotalApps ? grpPriorApprovals / grpPriorTotalApps : 0;

    const grpTotalDisb = totDisb;
    ...
    const appTotalRow = `<tr class="total-row">
      ...
      <td>${fmtN(grpTotalTeams)}</td>
      <td>${fmtN(grpTotalApps)}</td>
      <td>${fmtP(grpAppRate)}</td>
      <td class="${bpsClass(grpAppRate, grpPriorAppRate, false)}">${fmtB(grpAppRate, grpPriorAppRate)}</td>
    </tr>`;
    ...
    const defTotalRow = `<tr class="total-row">
      ...
      <td class="${bpsClass(grpChronic, grpPriorChronic, true)}">${fmtB(grpChronic, grpPriorChronic)}</td>
      <td>${fmtP(grpPar)}</td>
      <td class="${bpsClass(grpPar, grpPriorPar, true)}">${fmtB(grpPar, grpPriorPar)}</td>
    </tr>`;
    ...
    <div class="month-page" id="page-${monthStr}">
      ...
      <div class="table-block">
        ...
        <div class="tbl-wrap"><table>
          <thead><tr><th>Country</th><th>Apps / Team</th><th>vs Prior Month</th><th>Teams</th><th>Total Apps</th><th>Approval Rate</th><th>Change (bps)</th></tr></thead>
