...
async function buildPptx(monthStr, data) {
  ...
  // ── SLIDE 2: LOAN APPLICATIONS ──────────────────────────────────────────
  {
    ...
    barChart(s, 0.15, 0.92, 8.3, 3.8, CC.map(c => CC_NAMES[c]), appsVals, C.slate, false);
    sectionLabel(s, 0, 4.75, 8.5, "% Change from Prior Month");
    changeRow(s, appsChgs, appsColors, 5.08, 8.3 / 5, 0.15);
  ...
  // ── SLIDE 3: ASSESSMENT EFFICIENCY ──────────────────────────────────────
  {
    ...
    barChart(s, 0.15, 0.92, 8.3, 3.8, CC.map(c => CC_NAMES[c]), effVals, C.teal, true);
    sectionLabel(s, 0, 4.75, 8.5, "Change from Prior Month (bps)");
    changeRow(s, effBps, effColors, 5.08, 8.3 / 5, 0.15);
  ...
  // ── SLIDE 4: DISBURSEMENTS ───────────────────────────────────────────────
  {
    ...
    barChart(s, 0.15, 0.92, 8.3, 3.8, CC.map(c => CC_NAMES[c]), disbVals, C.slate, false);
    sectionLabel(s, 0, 4.75, 8.5, "% Change from Prior Month");
    changeRow(s, disbChgs, disbColors, 5.08, 8.3 / 5, 0.15);
  ...
  // ── SLIDE 5: AVG LOAN SIZE & DISB PER TEAM ──────────────────────────────
  {
    ...
    barChart(s, 0.15, 0.92, 6.35, 3.8, CC.map(c => CC_NAMES[c]), loanVals, C.slate, false);
    barChart(s, 6.75, 0.92, 6.35, 3.8, CC.map(c => CC_NAMES[c]), disbTeamVals, C.teal, false);

    sectionLabel(s, 0, 4.75, 6.55, "% Change from Prior Month");
    changeRow(s, loanChgs, loanColors, 5.08, 6.35 / 5, 0.15);

    sectionLabel(s, 6.75, 4.75, 6.4, "% Change from Prior Month");
    changeRow(s, disbTeamChgs, disbTeamColors, 5.08, 6.35 / 5, 6.75);
  }
  ...
  // ── SLIDE 6: LOAN BOOK ───────────────────────────────────────────────────
  {
    ...
    barChart(s, 0.15, 0.92, 8.3, 3.8, CC.map(c => CC_NAMES[c]), bookVals, C.slate, false);
    sectionLabel(s, 0, 4.75, 8.5, "% Change from Prior Month");
    changeRow(s, bookChgs, bookColors, 5.08, 8.3 / 5, 0.15);
  ...
  for (const { title, key, priorKey, color } of opexSlides) {
    ...
    const vals  = CC.map((cc, i) => teams[i] ? Math.round((d[cc]?.[key] || 0) / teams[i]) : 0);
    const priorVals = CC.map((cc, i) => teams[i] ? Math.round((d[cc]?.[priorKey] || 0) / teams[i]) : 0);
